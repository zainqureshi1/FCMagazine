...
public class FoldableListLayout extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            setOrientation(attrs.getAttributeIntValue("http://schemas.android.com/apk/res/android",
                    "orientation", LinearLayout.VERTICAL));
        }

        screenSize = Utility.getScreenSize(context);
        ...
        gestureDetector = new GestureDetector(context, new SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velX, float velY) {
                if (orientation == VERTICAL) {
                    return FoldableListLayout.this.onFling(e1, velY);
                } else {
                    return FoldableListLayout.this.onFling(e1, velX);
                }
    ...
    private FoldableItemLayout getLayoutForItem(int position) {
        ...
        if (layout == null) {
            ...
            layout = new FoldableItemLayout(getContext(), orientation);
    ...
    public void scrollToPosition(int index, long duration) {
        index = Math.max(0, Math.min(index, getCount() - 1));
        animateFold(index * 180f, duration);
    }
    ...
    protected void animateFold(float to, long duration) {
        final float from = getFoldRotation();
    ...
    private boolean processTouch(MotionEvent event) {
        ...
        if (getCount() > 0) {
            ...
            if (orientation == VERTICAL) {
                ...
                eventCopy.offsetLocation(getTranslationX(), 0);
            }
    ...
    private int touchStartSection(MotionEvent firstEvent) {
        int section = 1;
        if (firstEvent.getAction() == MotionEvent.ACTION_DOWN) {
            if (orientation == VERTICAL) {
                float touchY = firstEvent.getY();
                if (touchY <= screenSize.y*.2) {
                    section = 0;
                } else if (touchY >= screenSize.y*.8) {
                    section = 2;
                }
            } else {
                float touchX = firstEvent.getX();
                if (touchX <= screenSize.x*.2) {
                    section = 0;
                } else if (touchX >= screenSize.x*.8) {
                    section = 2;
                }
            }
        }
    ...
    private boolean onScroll(MotionEvent firstEvent, MotionEvent moveEvent) {
        int firstSection = touchStartSection(firstEvent);
        if (firstSection == 1) {
            // Cannot start from middle of screen
            return false;
        }
        if (orientation == VERTICAL) {
            if (!isScrollDetected && getHeight() != 0) {
                float firstDistance = firstEvent.getY() - moveEvent.getY();
                boolean directionOk = (firstSection == 0 && firstDistance < 0) || (firstSection == 2 && firstDistance > 0);
                if (Math.abs(firstDistance) > minDistanceBeforeScroll && directionOk) {
            ...
            if (!isScrollDetected && getHeight() != 0) {
                float firstDistance = firstEvent.getX() - moveEvent.getX();
                boolean directionOk = (firstSection == 0 && firstDistance < 0) || (firstSection == 2 && firstDistance > 0);
                if (Math.abs(firstDistance) > minDistanceBeforeScroll && directionOk) {
                    isScrollDetected = true;
                    scrollStartRotation = getFoldRotation();
                    scrollStartX = moveEvent.getX();
                }
            }

            if (isScrollDetected) {
                float distance = scrollStartX - moveEvent.getX();
                float rotation = 180f * scrollFactor * distance / getWidth();
                setFoldRotation(scrollStartRotation + rotation, true);
            }
        }
    ...
    private boolean onFling(MotionEvent firstEvent, float velocity) {
        int firstSection = touchStartSection(firstEvent);
        if (firstSection == 1) {
            // Cannot start from middle of screen
            return false;
        }
        boolean directionOk = (firstSection == 0 && velocity > 0) || (firstSection == 2 && velocity < 0);
        if (!directionOk) {
            return false;
        }
        ...
        if (orientation == VERTICAL) {
            ...
            relativeVelocity = -velocity / getHeight() * 180f;
        } else {
            if (getWidth() == 0) {
                return false;
            }

            relativeVelocity = -velocity / getWidth() * 180f;
        }
        relativeVelocity = Math.max(MIN_FLING_VELOCITY, Math.abs(relativeVelocity))
                * Math.signum(relativeVelocity);

        return flingAnimation.fling(relativeVelocity);
    }
    ...
    private class FlingAnimation implements Runnable {
        ...
        private void startInternal() {
            Utility.postOnAnimation(FoldableListLayout.this, this);
            isAnimating = true;
        }
